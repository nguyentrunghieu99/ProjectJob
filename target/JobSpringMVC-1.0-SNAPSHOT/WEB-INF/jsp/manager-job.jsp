            ...
            <div>
                <ul class="pagination">
                    <c:forEach begin="1" end="${Math.ceil(counter/maxIn)}" var="i">
                        <li class="page-item"><a class="page-link" href="<c:url value="/admin/jobs"/>?kw=${param.kw}&page=${i}&cat=${param.cat}&loca=${param.loca}">${i}</a></li>
                        </c:forEach>
                </ul>
            </div>
                        ...
                        <a href="<c:url value="/jobs/"/>${j.id}" class="c-itemJob">
                            ...
                            <div class="c-itemJob__info my-date" >
                                ...
                                <form method="post" action="/JobSpringMVC/td/detail/applications">
                                    ...
                                    <input type="submit" value="Đơn ứng tuyển" class="format-button"/>
                                </form>
                                    <br>
                                <form method="post" action="/JobSpringMVC/admin/jobs">
                                    <input type="hidden" name="deleteJob" value="${j.id}">
                                    <input type="submit" value="Xóa bài" class="format-button"/>
                                </form>
                            </div>
                        </a>
                    </c:forEach>
